...
public class HeartFeedback : MonoBehaviour
{
    ...
    public void SetupAndPlay(Vector3 spawnPosition)
    {
        ...
        // Move tween
        sequence.Append(transform.DOLocalMoveY(moveDistance, tweenTime).SetRelative().SetEase(Ease.OutQuad));

        // Scale tween
        sequence.Join(transform.DOScale(transform.localScale * scaleMultiplier, tweenTime).SetEase(Ease.OutBack));

        // Rotation tween
        sequence.Join(transform.DORotate(new Vector3(-90, 0, rotationAngle), tweenTime, RotateMode.FastBeyond360).SetEase(Ease.OutQuad));
